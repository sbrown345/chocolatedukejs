...
function parascan(dax1, dax2, sectnum,  dastat, bunch) {
    ...
    for(z=bunchfirst[bunch]; z>=0; z=bunchWallsList[z]) {
        ...
        else j = (sector[nextsectnum] || new SectorType()).floorstat;
...
function drawline256 ( x1,  y1,  x2,  y2,   col)
{
    var dx, dy, i, j, inc, plc, daend;
    var p;
    col = palookup[0][col];
    ...
    if (klabs(dx) >= klabs(dy))
    {
        ...
        for(; i<daend; i++)
        {
            ...
            if ((j >= startumost[i]) && (j < startdmost[i])) {
                frameplace.array[ylookup[j] + i] = col;
            }
            plc += inc;
        }
    }
    else
    {
        ...
        i = ((y1+2048)>>12);
        daend = ((y2+2048)>>12);
        p = new PointerHelper(frameplace.array, ylookup[i]);
        for(; i<daend; i++)
        {
            j = (plc>>12);
            if ((i >= startumost[j]) && (i < startdmost[j])) {
                p.setByteOffset(col, j);
            }
            plc += inc;
...
function drawmasks() {
    ...
    if(spritesortcnt>0)printf("1tspriteptr[1].picnum %i\n", tspriteptr[1].picnum);
    ...
    ys = spritesy[0];
    i = 0;
    if(spritesortcnt>0)printf("2tspriteptr[1].picnum %i\n", tspriteptr[1].picnum);
    ...
    if(spritesortcnt>0)printf("3tspriteptr[1].picnum %i\n", tspriteptr[1].picnum);
